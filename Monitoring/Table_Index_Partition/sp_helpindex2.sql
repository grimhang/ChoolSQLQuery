...
BEGIN
	...
	SELECT 
		...
		,LEFT(CAST(SUBSTRING(B.INDEX_COLUMN_DESC, 3, 1000) AS NVARCHAR(500)), 200) AS IndexColumn       -- [2023-03-06 여기해결] XML결과값은 텍스트모드에서 무한정 길어지는 문제 해결
		,LEFT(CAST(SUBSTRING(C.INCLUDE_COLUMN_DESC, 3, 1000) AS NVARCHAR(500)), 200) AS IncludeColumn
		,STATS_DATE(i.object_id, i.index_id) AS [STATISTICUPDATEDATE]
		,I.IS_UNIQUE
		,I.IS_UNIQUE_CONSTRAINT
		,I.IS_PRIMARY_KEY
		,I.HAS_FILTER
		,LEFT(CAST(I.FILTER_DEFINITION AS NVARCHAR(500)), 200) AS FILTER_DEFINITION
		,I.IS_DISABLED
		,STAT1.*
		--,STAT2.*
	--,STAT3.*
